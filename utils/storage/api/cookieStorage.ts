...
import type { CookieStorage } from '../type'
...
const Cookie: CookieStorage = {
  ...
  set: (
    ...
    const value = isObject(object) ? encodeURIComponent(JSON.stringify(object)) : object

    const samesite = sameSite === '' ? '' : `${sameSite};`

    document.cookie = `${encodeURIComponent(
      key
    )}=${value};path=${path};expires=${exp};${samesite}${secure}`
  }
}
